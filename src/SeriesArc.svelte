...
{#if !bg}
  {#if stackSeries}
    <path
      d={getPercArcPath(50, 50, $store.series[serieIdx].radius, $store.series[serieIdx].prevOffset, $store.series[serieIdx].prevOffset + $store.series[serieIdx].perc)}
      fill="transparent"
      stroke={$store.series[serieIdx].color}
      opacity={$store.series[serieIdx].opacity}
      stroke-width={thickness}
      class="pb-arc" />
  {:else}
    <path
      d={getPercArcPath(50, 50, $store.series[serieIdx].radius, 0, $store.series[serieIdx].perc)}
      fill="transparent"
      stroke={$store.series[serieIdx].color}
      stroke-width={thickness}
      opacity={$store.series[serieIdx].opacity}
...
{:else}
  <path
    d={getPercArcPath(50, 50, $store.series[serieIdx].radius, 0, 100)}
    fill="transparent"
    stroke={$store.series[serieIdx].color}
    stroke-width={thickness}
    opacity={$store.series[serieIdx].opacity}
