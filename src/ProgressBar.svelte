<script lang="ts">
  ...
  if (ry == null) ry = rx;
  export let colors: Array<string> = ["#FFC107", "#4CAF50", "#03A9F4"];
  export let opacities: Array<number>;
  ...
  export function updateOpacity(opacity: number, seriesIdx: number = 0): void {
    if (!Array.isArray(series)) series = [series];

    if (!series[seriesIdx] || typeof series[seriesIdx] != "object") {
      series[seriesIdx] = { opacity };
    } else {
      series[seriesIdx].opacity = opacity;
    }
  }
...
<svelte:options accessors={true} />{#if style == 'radial'}
  <RadialProgressBar
    {store}
    {colors}
    {thresholds}
