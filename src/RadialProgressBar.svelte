<script lang="ts">
  ...
  const maskStore = seriesStore(maskSeries, {
    colors,
    thresholds,
    stackSeries: false,
    thickness,
...
<svg
  ...
  {#if addBackground && stackSeries}
    <Arc
      radius={$maskStore.series[0].radius}
      fill={bgFillColor}
      {startAngle}
      {endAngle}
  ...
  {#if showProgressValue}
    <foreignObject
      class="progress-value progress-value-inverted"
      x="0"
      y="0"
      width="100%"
      height="100%">
      <div
        class="progress-value-content"
        style="font-size:{textSize}%;color:{labelColor}">
        {@html $store.label}
      </div>
    </foreignObject>
    <foreignObject
